...
- `CompanionLifecycle` mirrors the thin Player2NPC lifecycle layer for Forge: when `companion.autoSummonCompanion` is enabled, player login requests assigned PlayerEngine characters and ensures each session companion exists near the player; when `companion.dismissAutoSummonedCompanionsOnLogout` is enabled, only companions created by that automatic session lifecycle are dismissed on logout. Manual `/staywithme spawn` companions remain persistent unless explicitly dismissed with `/staywithme dismiss` or `/staywithme despawn`.
- `FriendMemory` now persists a Player2NPC-style profile-key to companion-entity UUID map. `CompanionLifecycle` uses that map before radius search in the player's current dimension, refreshes it when a companion is spawned or rediscovered, and removes stale or cross-dimension entries when companions are dismissed or no longer valid.
...
- `FriendEntity` and `FriendAutomatoneBridge` now expose and synchronize visible equipment instead of clearing it. The selected mainhand stack is mirrored through entity data for client rendering, `getHandSlots()` reads the same unified slot accessors, and the offhand/armor slots remain backed by entity equipment or the PlayerEngine bridge, matching Player2NPC's player-like mainhand/offhand/armor inventory shape more closely.
