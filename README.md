...
- `FriendEntity` and `FriendAutomatoneBridge` now expose and synchronize visible equipment instead of clearing it. The selected mainhand stack is mirrored through entity data for client rendering, and the offhand/armor slots remain backed by entity equipment or the PlayerEngine bridge, matching Player2NPC's player-like mainhand/offhand/armor inventory shape more closely.
