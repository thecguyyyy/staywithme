...
- The Forge client now has a Player2NPC-style companion selector on `H` and `/staywithmecompanions`. It loads PlayerEngine characters through `CharacterUtils.requestCharacters(...)` using the same Player2 game id as Player2NPC, stores the selected character profile in StayWithMe memory, and sends summon/despawn requests through `CompanionCharacterActionPacket`.
