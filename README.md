...
- `FriendMemory` now persists a Player2NPC-style profile-key to companion-entity UUID map. `CompanionLifecycle` uses that map before radius search, refreshes it when a companion is spawned or rediscovered, and removes stale entries when companions are dismissed or no longer valid.
