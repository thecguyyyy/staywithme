...
- `FriendInteractionProvider`: unified interaction entry point for reach checks, block breaking, block placing, attacking, swinging, and PlayerEngine-backed block breaking/item use when the PlayerEngine provider entity is available and enabled.
...
- `CompanionLifecycle` mirrors the thin Player2NPC lifecycle layer for Forge: when `companion.autoSummonCompanion` is enabled and PlayerEngine is loaded/enabled, player login requests assigned PlayerEngine characters and ensures each session companion exists near the player. Empty or failed assigned-character requests do not auto-spawn a default memory companion. When PlayerEngine is missing, the old default companion auto-summon remains as a Forge fallback. When `companion.dismissAutoSummonedCompanionsOnLogout` is enabled, only companions created by that automatic session lifecycle are dismissed on logout. Manual `/staywithme spawn` companions remain persistent unless explicitly dismissed with `/staywithme dismiss` or `/staywithme despawn`.
- `FriendMemory` now persists a Player2NPC-style profile-key to companion-entity UUID map. `CompanionLifecycle` uses that map before radius search in the player's current dimension, refreshes it when a companion is spawned or rediscovered, removes stale or cross-dimension entries when companions are dismissed or no longer valid, and reconciles remembered entries against the latest assigned PlayerEngine profiles so no-longer-assigned companions are dismissed after a profile-list change. An empty assigned-character list is now treated as authoritative and clears previously session-managed PlayerEngine companions instead of leaving stale entities alive.
- `CompanionLifecycle.getActiveCompanions(...)` mirrors Player2NPC's companion-manager active-companion enumeration by resolving tracked session and persisted profile-key UUIDs across loaded server levels, dropping invalid remembered UUID strings, and refreshing the session map for live owned companions. `/staywithme companions` reports that tracked active companion set for debugging.
- `CompanionLifecycle.dismissAllCompanions(...)` mirrors Player2NPC's tracked-map `dismissAllCompanions()` behavior for Forge: it dismisses only companions tracked by the session/persisted profile-key UUID maps, clears that persisted map, and verifies the target entity is owned by the requesting player before discarding it. `/staywithme dismissall` exposes this cleanup path.
- Selected-character despawn uses that persisted profile-key to entity-UUID map before nearby search, so a remembered PlayerEngine companion can be dismissed by UUID after reload or when it is outside the local search radius.
- Spawn and recall placement now uses randomized nearby standable blocks around the owner, with a Player2NPC-style adjacent y+1 random fallback only when no safe nearby stand position is found.
- `CompanionLifecycle` applies `FriendMemory.companionName` as the entity display name on spawn/recall, and `FriendEntity.getDisplayName()` now falls back to the persisted companion profile display name, mirroring Player2NPC's character-derived companion display name while keeping `Companion` as the default.
- `PlayerEngineCompanionCharacter` now builds the PlayerEngine `Character` from `FriendMemory.companionName` and the owner name, so `FriendPlayerEngineController` initializes PlayerEngine with the StayWithMe companion identity instead of `CharacterUtils.DEFAULT_CHARACTER`.
- PlayerEngine conversation lifecycle is initialized in the Forge layer: `PlayerEngineGlobalTickBridge` ensures the packaged PlayerEngine global server tick hook is registered, and `FriendPlayerEngineController` sends the same greeting event Player2NPC sends after controller creation.
- Selected-character summon/despawn packets are resolved through the server's current PlayerEngine assigned-character list when PlayerEngine is loaded and enabled. The server uses the canonical assigned profile for memory/lifecycle updates, rejects stale or unassigned client profiles, and ignores late validation results if the sender logs out before the async request returns.
- The Forge client now has a Player2NPC-style companion selector on `H` and `/staywithmecompanions`. It loads PlayerEngine characters through `CharacterUtils.requestCharacters(...)` using the same Player2 game id as Player2NPC, stores the selected character profile in StayWithMe memory, shows skin-head character cards plus a selected-character detail head, and sends selected-character summon/despawn requests through `CompanionCharacterActionPacket`. Multiple selected PlayerEngine characters can coexist; each entity persists its own profile for PlayerEngine controller initialization, syncs its skin URL to clients, and renders that skin through a small downloaded-texture cache. The companion renderer now uses a player model with armor, arrow, spin-attack, and stinger layers so PlayerEngine equipment changes are visible like a Player2NPC-style player-shaped NPC.
- `FriendEntity` now uses Forge `IEntityAdditionalSpawnData` plus `NetworkHooks.getEntitySpawningPacket(...)` as the Forge equivalent of Player2NPC's custom `AutomatonSpawnPacket`: newly spawned clients receive the companion profile plus mainhand/offhand/armor snapshots in the spawn payload so the first rendered frame has the selected character skin/name context and visible equipment.
- `FriendEntity` and `FriendAutomatoneBridge` now expose and synchronize visible equipment instead of clearing it. The selected mainhand stack is mirrored through entity data for client rendering, `getHandSlots()` and `getArmorSlots()` read the same unified slot accessors, and the offhand/armor slots remain backed by entity equipment or the PlayerEngine bridge, matching Player2NPC's player-like mainhand/offhand/armor inventory shape more closely.
- `FriendAutomatoneBridge` resolves armor/offhand sync with per-tick snapshots: PlayerEngine equipment changes write back to entity equipment for rendering/persistence, while external vanilla entity-equipment changes are imported only when PlayerEngine left that slot unchanged.
- `PlayerEngineFriendEntity` now resolves server-side mainhand, offhand, and armor slot reads from PlayerEngine `LivingEntityInventory` while `usePlayerEngineController` is enabled, and writes slot updates to both PlayerEngine inventory and the Forge-visible equipment/mirror. This is the Forge-side equivalent of Player2NPC's `getEquippedStack(...)` / `equipStack(...)` behavior; bridge initialization deliberately imports the pre-existing Forge equipment once before switching PlayerEngine inventory into the slot source of truth.
- `FriendInventoryProvider` now notifies the entity when local mirror slots or the selected slot change. `PlayerEngineFriendEntity` mirrors those writes into PlayerEngine `LivingEntityInventory` immediately, so StayWithMe fallback inventory mutations and PlayerEngine interaction managers see the same selected tool in the same tick.
- `PlayerEngineFriendEntity` now flushes PlayerEngine `LivingEntityInventory` back to the StayWithMe inventory mirror before saving, and stores a PlayerEngine-format inventory NBT snapshot while `usePlayerEngineController` is enabled. On reload it restores that PlayerEngine inventory first, then syncs the mirror back, matching Player2NPC's `LivingEntityInventory.writeNbt/readNbt` persistence model more closely. On death, PlayerEngine-enabled companions drop main, armor, and offhand through `LivingEntityInventory.dropAll()` once before clearing the mirror, matching Player2NPC's inventory-wide death drop path.
...
- `FriendEntity` automatic dropped-item pickup now skips items with an active pickup delay and emits the vanilla pickup feedback when it moves stacks into the companion inventory, matching Player2NPC's `cannotPickup()` guard more closely. When the active entity is `PlayerEngineFriendEntity` and `usePlayerEngineController` is enabled, pickup insertion now goes through PlayerEngine's `LivingEntityInventory.insertStack(...)` first and then syncs back to the StayWithMe inventory mirror.
...
- `FriendEntity.aiStep()` also keeps the rendered head yaw aligned to the entity yaw each tick, matching Player2NPC's `tickMovement()` `headYaw = getYaw()` behavior so PlayerEngine/Baritone-controlled movement does not leave the player-like model looking in a stale direction.
- `FriendEntity.knockback(...)` now mirrors Player2NPC's knockback guard by only accepting knockback while the entity is already marked as hurt/velocity-modified, reducing incidental displacement while PlayerEngine controls movement.
- `FriendEntity.attackAsCompanion(...)` centralizes local/provider melee attacks, sets the living target, delegates damage/enchantment handling to Forge `Mob#doHurtTarget(...)`, and triggers a main-hand swing so fallback attacks render like Player2NPC's player-like attack loop.
- `FriendInteractionProvider` now optionally delegates block breaking and block item use to PlayerEngine's `LivingEntityInteractionManager` through reflection when the companion is a PlayerEngine provider entity and `usePlayerEngineController` is enabled, preserving pure Forge fallback loading when PlayerEngine is absent or disabled. The block-breaking path keeps StayWithMe's existing reach, block-entity, and correct-tool guards, then uses PlayerEngine's `processBlockBreakingAction(...)` for progress, drops, and tool damage before falling back to `SurvivalWorldInteractor` when the provider is unavailable.
...
- `FriendRenderer` now mirrors Player2NPC's render guard by logging and swallowing companion render exceptions instead of letting one bad companion render crash the client frame.
