...
- `FriendMemory` schema 7 persists `companionEntityUuids` as profile-key to entity-UUID strings. `CompanionLifecycle` resolves that remembered UUID across all server levels before falling back to nearby search/spawn, updates it when a session companion is spawned or rediscovered, and removes entries on dismiss or stale/invalid lookup.
