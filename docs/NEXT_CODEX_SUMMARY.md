...
- Torch restock blocked-state detection now considers torch crafting materials in the supply chest. If the chest has coal/charcoal plus sticks or enough planks to craft sticks, the companion does not mark torch restock unavailable just because there are no finished torch stacks.
...
- Local branch mining now has a first-pass branch pattern instead of only walking a single straight tunnel:
  ...
  - alternates side branches left/right
