...
- `StayWithMeCompanionScreen` reuses `CompanionSkinTextures` to render the selected character's face and hat overlay in the detail panel, matching Player2NPC's character-detail skin-head presentation without serializing PlayerEngine classes through StayWithMe packets.
