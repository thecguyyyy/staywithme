...
- Last verified result: build passes on 2026-06-21 after adding Player2NPC-style `FriendEntity` entity registration (`MobCategory.MISC`, player width/height, 64-block client tracking, 1-tick update interval), PlayerEngine-backed `CLEAR_LIQUID`, automatic passage-fluid clearing for ordinary resource exploration and mining-expedition tunnel digging tracked through `PlayerEngineBlockSafetyRunner`, PlayerEngine `GetToYTask` as the first attempt for mining-expedition `DESCEND_TO_LAYER`, one-shot PlayerEngine `GetToBlockTask` attempts before remote/stalled return travel falls back to construction-route recovery, Player2NPC-style companion join/logout lifecycle and memory-backed display naming through `CompanionLifecycle`, Player2NPC-style PlayerEngine `Character` creation through `PlayerEngineCompanionCharacter`, Player2NPC-style PlayerEngine conversation lifecycle initialization through `PlayerEngineGlobalTickBridge` and `ConversationManager.sendGreeting(...)`, Player2NPC-style H-key companion selection through PlayerEngine `CharacterUtils.requestCharacters(...)`, Player2NPC-style selected-character spawn/despawn client requests through `CompanionCharacterActionPacket`, Player2NPC-style assigned-character login synchronization through `PlayerEngineCharacterProfiles`, per-entity persisted companion profiles for multiple simultaneous PlayerEngine companions, Player2NPC-style offhand equipment exposure/sync through `FriendEntity` and `FriendAutomatoneBridge`, a vanilla charcoal workflow fallback for `COLLECT_FUEL`, limited Forge fallbacks for `GET_OUT_OF_WATER` and `ESCAPE_LAVA`, limited carried-armor fallback for `EQUIP_ARMOR`, limited local retreat fallback for `RETREAT_FROM_HOSTILES`, limited local sidestep fallback for `DODGE_PROJECTILES`, and limited carried-block cover fallback for `PROJECTILE_PROTECTION_WALL`. The carried-armor fallback is now split into `LocalArmorEquipFallback`, local dry/lava-safe stand search is split into `LocalHazardEscapeFallback`, local liquid/fire block handling is split into `LocalBlockSafetyFallback`, local hostile/projectile safety fallback is split into `LocalThreatSafetyFallback`, carried food/meat/fuel satisfaction checks, best carried-food selection, basic material/tool inventory counters, construction floor-repair material filtering, supply-furnace block selection, free-slot counting, generic supply-container item count/move/overflow policy helpers, supply-stock status formatting, and expedition pickaxe usability/durability checks are split into `LocalInventoryFallback`, item alias/inventory/crafting target resolution is split into `LocalItemMatcher`, reload-resume PlayerEngine/fallback availability checks are split into `PlayerEngineResumeValidator`, PlayerEngine movement starts are split into `PlayerEngineMovementRunner`, PlayerEngine armor equip execution is split into `PlayerEngineArmorEquipRunner`, PlayerEngine water/lava escape execution is split into `PlayerEngineHazardEscapeRunner`, PlayerEngine block placement execution is split into `PlayerEnginePlaceBlockRunner`, PlayerEngine liquid/fire safety execution is split into `PlayerEngineBlockSafetyRunner`, PlayerEngine threat-safety execution is split into `PlayerEngineThreatSafetyRunner`, PlayerEngine hostile attack/protect execution is split into `PlayerEngineHostileAttackRunner`, PlayerEngine route-block/food/meat collection execution is split into `PlayerEngineCollectionRunner`, PlayerEngine furnace smelting execution is split into `PlayerEngineSmeltingRunner`, PlayerEngine fish/farm/sleep routine execution is split into `PlayerEngineRoutineRunner`, PlayerEngine item pickup/give/deposit execution is split into `PlayerEngineItemTransferRunner`, PlayerEngine remote return/goto attempts are split into `PlayerEngineRemoteTravelRunner`, PlayerEngine Y-layer travel attempts are split into `PlayerEngineYLevelRunner`, PlayerEngine exploration execution is split into `PlayerEngineExploreRunner`, PlayerEngine fuel collection and charcoal fallback routing is split into `PlayerEngineFuelRunner`, PlayerEngine construction-material restock is split into `PlayerEngineConstructionMaterialRestockRunner`, shared construction travel results are split into `ConstructionTravelResult`, PlayerEngine catalogue request mapping is split into `PlayerEngineAcquisitionRequests`, common PlayerEngine get/acquisition execution is split into `PlayerEngineAcquisitionRunner`, counted PlayerEngine task execution is split into `PlayerEngineCountedTaskRunner`, start-only continuous PlayerEngine execution is split into `PlayerEngineStartOnlyTaskRunner`, callback-confirmed PlayerEngine execution is split into `PlayerEngineConfirmedTaskRunner`, PlayerEngine task-with-fallback execution is split into `PlayerEngineFallbackTaskRunner`, PlayerEngine status truncation is split into `PlayerEngineStatusText`, and high-level PlayerEngine task state/start/announcement handling is split into `PlayerEngineTaskState`, so `LocalBehaviorController` can keep shrinking toward PlayerEngine-first orchestration. This sits on top of PlayerEngine-backed `FOLLOW_PLAYER`/`GO_TO_POSITION`/`RETURN_TO_PLAYER`, `GET_ITEM`, `PICKUP_DROPPED_ITEM`, `COLLECT_BUILDING_MATERIALS`, construction-route material restock, `GIVE_ITEM`/`DEPOSIT_INVENTORY`, `SMELT_ITEM`, `EXPLORE`, `ATTACK_NEARBY_HOSTILE`/`PROTECT_PLAYER`/`RETREAT_FROM_HOSTILES`/`RETREAT_FROM_CREEPERS`/`DODGE_PROJECTILES`/`PROJECTILE_PROTECTION_WALL`, `SLEEP_THROUGH_NIGHT`/`GET_OUT_OF_WATER`/`ESCAPE_LAVA`/`PUT_OUT_FIRE`, `COLLECT_FOOD`/`COLLECT_MEAT`/`COLLECT_FUEL`, `FISH`/`FARM`/`EQUIP_ARMOR`, `/staywithme capabilities`, PlayerEngine-style command broadening, smarter catalogue plural aliases, generic-get mining fallback, local parser quantity cleanup, and acquisition failure diagnostics.
- Latest focused pass: `ModEntities.FRIEND` now registers the companion as `MobCategory.MISC` with `EntityType.PLAYER` width/height, 64-block client tracking, and 1-tick update interval, matching the Player2NPC entity registration shape where Forge exposes equivalent builder options. `FriendMemory` now persists a Player2NPC-style profile-key to companion-entity UUID map, and `CompanionLifecycle` uses it before radius search so selected PlayerEngine companions can be reused after reload instead of relying only on in-memory session state. `FriendEntity` fixes its main arm to `HumanoidArm.RIGHT` and reports `isLeftHanded=false`, matching Player2NPC's right-handed companion baseline instead of inheriting random Mob handedness. `FriendEntity.getDisplayName()` returns the persisted companion profile display name when the entity has a PlayerEngine/companion identity, matching Player2NPC's character-derived display-name override even if custom-name data is missing. `FriendEntity` automatic dropped-item pickup skips items with an active pickup delay and emits vanilla pickup feedback when moving stacks into the companion inventory, matching Player2NPC's `cannotPickup()` guard more closely. `CompanionLifecycle` spawns or recalls companions to randomized nearby standable blocks around the owner, with a Player2NPC-style adjacent y+1 random fallback only when no safe nearby stand position is found. `FriendEntity` applies Player2NPC-style base movement constants: max step height `0.6F` and movement speed `0.4D`, matching Player2NPC's `AutomatoneEntity` baseline before higher-level navigation. The Player2NPC-style companion selector uses skin-head character cards and renders the selected character's skin head in the detail area using the same downloaded-texture cache as entity rendering. Visible equipment sync mirrors the selected mainhand `ItemStack` through `FriendEntity` entity data so clients render the companion's current carried tool/item even though the real inventory remains server-side. Character skins are synced through entity data and rendered on the client through `CompanionSkinTextures`, with default-skin fallback when a selected PlayerEngine character has no valid skin URL. `FriendRenderer` uses `PlayerModel` plus armor, arrow, spin-attack, and stinger layers so equipped armor and skin outer layers render closer to Player2NPC's `RenderAutomaton`.
- Latest renderer alignment: `FriendRenderer` now mirrors Player2NPC/Forge `PlayerRenderer` model setup for selected companions: visible outer skin parts, main/offhand arm poses for item use and charged crossbows, Forge custom arm-pose support, crouch render offset, 0.9375 player-scale rendering, and fall-flying/swimming rotations. Build passes on 2026-06-21.
- Latest render-safety alignment: `FriendRenderer.render(...)` now mirrors Player2NPC's `RenderAutomaton` guard by logging companion render exceptions and preventing a single bad companion render from crashing the client frame. Build passes on 2026-06-21.
- Latest equipment-slot alignment: `FriendEntity.getHandSlots()` now reads through `getItemBySlot(...)`, so client-side hand-slot enumeration uses the same synchronized mainhand entity data as renderer slot lookups. Build passes on 2026-06-21.
- Latest lifecycle-dimension alignment: `CompanionLifecycle` now resolves remembered/session companion UUIDs only in the player's current `ServerLevel`, matching Player2NPC's current-world reuse behavior. If a remembered UUID points to another dimension or an invalid entity, the old entity is discarded, memory is cleared, and the current dimension can spawn a fresh companion instead of moving coordinates in the wrong level. Build passes on 2026-06-21.
- Latest entity-state alignment: `FriendEntity` now persists `head_yaw` like Player2NPC and tracks the previous tick delta movement so `FriendRenderer` can use Player2NPC-style interpolated velocity for fall-flying rotation. Build passes on 2026-06-21.
- Latest movement alignment: `FriendEntity.aiStep()` now mirrors Player2NPC's water-sneak descent behavior by calling Forge's vanilla downward swimming impulse while the companion is in water and sneaking. Build passes on 2026-06-21.
- Latest head-yaw movement alignment: `FriendEntity.aiStep()` now keeps `yHeadRot` aligned to entity yaw after movement, matching Player2NPC's `tickMovement()` `headYaw = getYaw()` behavior so PlayerEngine/Baritone-controlled movement does not leave the player-like model looking in a stale direction. Build passes on 2026-06-22.
- Latest lifecycle-map alignment: `CompanionLifecycle.syncSessionCompanionsFor(...)` now reconciles persisted `FriendMemory.companionEntityUuids` against the latest assigned PlayerEngine profiles, dismissing and forgetting remembered companions that are no longer assigned before ensuring the current assigned set. `StayWithMeLifecycleEvents` also ignores async PlayerEngine character results after the joining `ServerPlayer` has left, preventing logout races from spawning stale companions. Build passes on 2026-06-22.
- Latest companion-request alignment: `CompanionCharacterActionPacket` now treats client selected-character summon/despawn as a Player2NPC-style request against the owner's PlayerEngine assignments instead of blindly trusting the serialized client profile. When PlayerEngine is loaded/enabled and the packet profile has an identity, the server asynchronously reloads assigned PlayerEngine profiles, matches by profile key, uses the canonical server-side profile for memory and lifecycle actions, rejects stale/unassigned profiles, and drops late validation results if the sender logged out. Empty-profile despawn still targets the remembered/default companion. Build passes on 2026-06-22.
- Latest knockback alignment: `FriendEntity.knockback(...)` now mirrors Player2NPC's `takeKnockback(...)` guard by only applying knockback while the entity is already marked as hurt/velocity-modified, reducing incidental displacement of the PlayerEngine-controlled companion body. Build passes on 2026-06-22.
- Latest equipment-bridge alignment: `FriendAutomatoneBridge` now snapshots armor/offhand stacks at the beginning of each PlayerEngine tick and uses those snapshots to resolve two-way sync. If PlayerEngine changes armor/offhand during the tick, that new stack writes back to the entity for rendering and persistence; if PlayerEngine leaves a slot unchanged but vanilla/entity equipment changes externally, the bridge imports the entity stack. This fixes the old non-empty armor conflict where an existing entity armor piece could overwrite a PlayerEngine `EquipArmorTask` upgrade before it rendered. Build passes on 2026-06-22.
- Latest assigned-character lifecycle alignment: `StayWithMeLifecycleEvents` now mirrors Player2NPC more strictly when PlayerEngine is loaded and enabled: join lifecycle only summons assigned PlayerEngine characters. Character-request failures or an empty assigned-character list now produce a visible message and do not auto-spawn a default memory companion. The default companion auto-summon remains only for the no-PlayerEngine fallback path and manual `/staywithme spawn`. Build passes on 2026-06-22.
- Latest persisted-despawn alignment: selected-character despawn now uses the persisted `FriendMemory.companionEntityUuids` profile-key map before falling back to nearby search, matching Player2NPC's persisted companion UUID map more closely. After a server restart or when a companion is outside the local search radius, `/staywithmecompanions` despawn can remove the remembered companion by UUID across server levels and clears stale/invalid remembered UUID entries. Build passes on 2026-06-22.
- Latest armor-slot alignment: `FriendEntity.getArmorSlots()` now enumerates armor through the same unified `getItemBySlot(...)` path used by renderer slot lookups and `FriendAutomatoneBridge`, matching Player2NPC's `getArmorItems() -> getLivingInventory().armor` behavior more closely so armor rendering/equipment enumeration reads the PlayerEngine-synchronized slots. Build passes on 2026-06-22.
- Latest empty-assignment lifecycle alignment: `CompanionLifecycle.syncSessionCompanionsFor(...)` now treats an empty PlayerEngine assigned-character list as an authoritative empty desired set. `StayWithMeLifecycleEvents` calls the sync even for empty lists, so remembered/session-managed PlayerEngine companions are dismissed and forgotten instead of lingering after all assignments are removed. Build passes on 2026-06-22.
- Latest active-companion manager alignment: `CompanionLifecycle.getActiveCompanions(...)` now mirrors Player2NPC's `CompanionManager.getActiveCompanions()` by resolving tracked session and persisted profile-key UUIDs across loaded server levels, filtering to live owned companions, dropping invalid remembered UUID strings, and refreshing the session map for live companions. `/staywithme companions` and `/staywithme activecompanions` report that tracked active set for debugging. Build passes on 2026-06-22.
- Latest dismiss-all lifecycle alignment: `CompanionLifecycle.dismissAllCompanions(...)` now mirrors Player2NPC's tracked-map `dismissAllCompanions()` behavior by dismissing all session/persisted profile-key UUID companions for the requesting owner and clearing the persisted map. UUID-targeted lifecycle dismissals now verify the resolved `FriendEntity` is owned by the requesting player before discarding it. `/staywithme dismissall` and `/staywithme dismissallcompanions` expose the tracked cleanup path. Build passes on 2026-06-22.
- Latest spawn-packet alignment: `FriendEntity` now implements Forge `IEntityAdditionalSpawnData` and returns `NetworkHooks.getEntitySpawningPacket(...)`, serving as the Forge equivalent of Player2NPC's custom `AutomatonSpawnPacket`. The spawn payload carries the companion profile plus mainhand/offhand/armor snapshots so newly spawned clients receive selected-character skin/name context and visible equipment immediately. Build passes on 2026-06-22.
- Latest README direction alignment: `README.md` now opens with the PlayerEngine-first + Player2NPC-style companion direction instead of the old LLM/local-survival executor framing. The first command section prioritizes companion selection, tracked companion lifecycle, PlayerEngine status/capability/catalogue debugging, and marks old crafting/mining/expedition flows as StayWithMe fallback or non-current-mainline strategy work. Docs-only change.
- Latest interaction-provider alignment: `FriendEntity.attackAsCompanion(...)` now centralizes fallback/provider melee attacks by setting the living target, delegating damage/enchantment behavior to Forge `Mob#doHurtTarget(...)`, and triggering a main-hand swing so local combat renders like Player2NPC's player-like attack loop. `FriendInteractionProvider.attackEntity(...)` and `DummyEmbodiedController.attack(...)` now use that path. `FriendInteractionProvider.useItemOnBlock(...)` also optionally delegates to PlayerEngine's `LivingEntityInteractionManager.interactBlock(...)` through reflection when the active entity is a PlayerEngine provider entity, preserving pure Forge fallback loading when PlayerEngine is absent. Build passes on 2026-06-23.
- Latest PlayerEngine interaction-manager breaking alignment: `FriendInteractionProvider.tickBreakBlock(...)` now prefers PlayerEngine's `LivingEntityInteractionManager.processBlockBreakingAction(...)` when the companion is a PlayerEngine provider entity and `usePlayerEngineController` is enabled. It keeps StayWithMe's existing reach, block-entity, and correct-tool guards, starts/stops PlayerEngine breaking progress, reports PlayerEngine tool-break events through the existing tool-break handling path, adds the same local mining exhaustion cost, and falls back to `SurvivalWorldInteractor` when PlayerEngine is unavailable, disabled, or the reflective provider call fails. Build passes on 2026-06-23.
- Latest PlayerEngine inventory-pickup alignment: `PlayerEngineFriendEntity.insertIntoInventory(...)` now mirrors Player2NPC's dropped-item pickup path by inserting picked-up stacks into PlayerEngine's `LivingEntityInventory.insertStack(...)` when `usePlayerEngineController` is enabled, then syncing that PlayerEngine inventory back to StayWithMe's local inventory mirror. Pure Forge fallback entities and PlayerEngine-disabled sessions still use `FriendEntity`'s existing inventory insertion. Build passes on 2026-06-23.
- Latest PlayerEngine inventory-persistence alignment: `PlayerEngineFriendEntity` now flushes PlayerEngine `LivingEntityInventory` back to the StayWithMe inventory mirror before saving, writes a PlayerEngine-format `PlayerEngineInventory` NBT snapshot plus selected slot while `usePlayerEngineController` is enabled, and restores that PlayerEngine inventory on reload before syncing the mirror back. This mirrors Player2NPC's `LivingEntityInventory.writeNbt/readNbt` persistence model more closely while preserving the Forge fallback save format. Build passes on 2026-06-23.
- Latest PlayerEngine inventory-death-drop alignment: `PlayerEngineFriendEntity.die(...)` now uses PlayerEngine `LivingEntityInventory.dropAll()` when `usePlayerEngineController` is enabled, dropping main inventory, armor, and offhand from the PlayerEngine inventory source of truth once before syncing the cleared inventory back to the StayWithMe mirror. Pure Forge fallback entities keep the existing `FriendEntity` inventory drop path. Build passes on 2026-06-23.
- Latest PlayerEngine equipped-slot source alignment: `PlayerEngineFriendEntity.getItemBySlot(...)` / `setItemSlot(...)` now resolve server-side mainhand, offhand, and armor through PlayerEngine `LivingEntityInventory` while `usePlayerEngineController` is enabled, matching Player2NPC's `getEquippedStack(...)` / `equipStack(...)` model more closely. Slot writes still update Forge-visible equipment or mirrored inventory for rendering and persistence, and bridge initialization now avoids recursive construction by assigning the bridge before the first one-time Forge-to-PlayerEngine sync. Build passes on 2026-06-23.
- Latest PlayerEngine inventory-mirror hook alignment: `FriendInventoryProvider` now notifies the entity when local mirror slots or the selected slot change, and `PlayerEngineFriendEntity` mirrors those writes into PlayerEngine `LivingEntityInventory.main` plus the selected hotbar slot while `usePlayerEngineController` is enabled. This keeps StayWithMe fallback inventory mutations from being overwritten by the next PlayerEngine sync and lets PlayerEngine interaction managers see the selected tool in the same tick. Build passes on 2026-06-23.
- Last upstream reference check during this development pass: Player2NPC `ec439c11b4fd31ebd3b1b698f28f632eec4081fc`, PlayerEngine `13fddc140a7ffd60b6bc9d1f084b36adc0674510`.
...
- `entity/CompanionLifecycle.java`
- `event/StayWithMeLifecycleEvents.java`
...
- `playerengine/PlayerEngineCompanionCharacter.java`
...
- `CompanionLifecycle` mirrors Player2NPC's thin `CompanionManager` lifecycle on Forge: login requests assigned PlayerEngine characters and ensures a session companion exists for each selected character when `companion.autoSummonCompanion=true`, and logout dismisses only session companions when `companion.dismissAutoSummonedCompanionsOnLogout=true`. Manual `/staywithme spawn` companions stay persistent unless explicitly dismissed with `/staywithme dismiss` or `/staywithme despawn`.
- `FriendMemory` schema 7 persists `companionEntityUuids` as profile-key to entity-UUID strings. `CompanionLifecycle` resolves that remembered UUID only in the player's current `ServerLevel` before falling back to nearby search/spawn, updates it when a session companion is spawned or rediscovered, and removes entries on dismiss, stale/invalid lookup, or cross-dimension mismatch.
- `CompanionLifecycle` uses randomized nearby standable blocks for spawn/recall placement instead of a fixed point in front of the player, and falls back to Player2NPC's adjacent y+1 random placement only if no safe nearby stand position is found.
- `CompanionLifecycle` applies `FriendMemory.companionName` as the entity display name on spawn/recall, and `FriendEntity.getDisplayName()` now falls back to the persisted companion profile display name when a PlayerEngine/companion identity exists, mirroring Player2NPC's character-derived companion display name while preserving `Companion` as the default.
- `PlayerEngineCompanionCharacter` builds the PlayerEngine `Character` from `FriendMemory.companionName` and the owner name, so `FriendPlayerEngineController` initializes `PlayerEngineController` with the StayWithMe companion identity instead of `CharacterUtils.DEFAULT_CHARACTER`.
- `FriendEntity` syncs each persisted companion profile's skin URL through vanilla entity data, and `FriendRenderer` resolves that URL through `CompanionSkinTextures` so selected PlayerEngine characters render with their Player2NPC-style downloaded skins on clients. `FriendRenderer` uses a player model plus armor/arrow/spin/stinger layers, preserving the existing head, elytra, and held-item layers from the humanoid renderer, applies Player2NPC/PlayerRenderer-style visible outer skin parts, use-action arm poses, crouch offset, 0.9375 player scale, and fall-flying/swimming rotations, and now logs/swallows companion render exceptions like Player2NPC's `RenderAutomaton`.
- `StayWithMeCompanionScreen` reuses `CompanionSkinTextures` to render character cards with face/hat overlays and the selected character's larger detail head, matching Player2NPC's character-card and character-detail skin-head presentation without serializing PlayerEngine classes through StayWithMe packets.
- `FriendEntity` and `FriendAutomatoneBridge` expose and synchronize visible equipment instead of clearing it. The selected mainhand stack is mirrored through entity data for client rendering, `getHandSlots()` reads through the same synchronized `getItemBySlot(...)` path, and offhand/armor stay wired through entity equipment or the PlayerEngine bridge, matching Player2NPC's player-like mainhand/offhand/armor equipment mapping more closely without exposing the whole server inventory.
- `FriendEntity` fixes `getMainArm()` to `HumanoidArm.RIGHT` and `isLeftHanded()` to `false`, matching Player2NPC's `getMainArm() -> Arm.RIGHT` behavior for player-like rendering and equipment semantics.
- `FriendEntity` persists `head_yaw` and tracks previous/current delta movement for renderer interpolation, matching Player2NPC's saved head-yaw and `lastVelocity.lerp(...)` render support without overriding existing look goals.
- `FriendEntity` automatic dropped-item pickup still respects `mobGriefing`, and now also skips `ItemEntity.hasPickUpDelay()` items before inserting stacks and calls `take(...)` after a successful move so client pickup feedback mirrors the Player2NPC item-pickup path more closely.
- `FriendEntity` sets max step height `0.6F` in its constructor and uses movement speed `0.4D`, matching Player2NPC's `AutomatoneEntity.init()` baseline movement shape without adding new path-planning semantics.
- `FriendEntity.aiStep()` mirrors Player2NPC's water-sneak descent behavior with Forge's vanilla downward swimming impulse when the companion is in water and sneaking.
- `ModEntities.FRIEND` now registers the companion as `MobCategory.MISC` with `EntityType.PLAYER` width/height, 64-block client tracking, and 1-tick update interval, matching the Player2NPC entity registration shape where Forge exposes equivalent builder options.
...
- First PlayerEngine-first refactor is implemented:
  - root `AGENTS.md` now documents Player2NPC and PlayerEngine as design references without requiring an upstream check before every edit
...
- Player2NPC-style conversation lifecycle alignment was added:
  - `StayWithMePlayerEngineEvents` initializes PlayerEngine's packaged global server tick hook through `PlayerEngineGlobalTickBridge`, matching the upstream need for `PlayerEngineController.staticServerTick(...)` without double-registering the Architectury callback
  - `FriendPlayerEngineController` sends `ConversationManager.sendGreeting(...)` when the companion's `PlayerEngineController` is first created, matching Player2NPC's controller initialization behavior
- Player2NPC-style client companion selection was added:
  - `H` and `/staywithmecompanions` open `StayWithMeCompanionScreen`, which loads PlayerEngine characters through `CharacterUtils.requestCharacters(...)` using the same Player2 game id as Player2NPC
  - `CompanionCharacterActionPacket` sends selected-character summon/despawn requests to the server without serializing PlayerEngine classes across the common packet boundary
  - selected character id/name/shortName/greeting/description/skin/voice ids are stored in `FriendMemory`, and `PlayerEngineCompanionCharacter` now uses those fields when creating the PlayerEngine `Character`
  - each `FriendEntity` now persists its own companion profile, so multiple PlayerEngine characters can coexist and initialize separate `PlayerEngineController` identities
  - login synchronization uses `PlayerEngineCharacterProfiles.requestCharacters(...)` to mirror Player2NPC assigned-character session summon behavior; when PlayerEngine is loaded/enabled, an empty or failed assigned-character request does not auto-spawn a default memory companion
...
- Recovered tasks are now validated before restart beyond ownership. Mining tasks must still target a supported executor resource, mining expeditions must still match the current dimension, and workflow tasks must be rebuildable before `FriendBrain.startTask()` is called. PlayerEngine-dependent resume checks now run through `PlayerEngineResumeValidator`, which allows already-satisfied or local-fallback-capable food, meat, fuel, smelting, water/lava escape, threat safety, pickup, building-material, and armor tasks to resume without PlayerEngine while still failing visibly when no valid fallback exists.
...
Result: successful on 2026-06-21 after adding Player2NPC-style entity registration through `ModEntities.FRIEND` (`MobCategory.MISC`, player width/height, 64-block client tracking, 1-tick update interval), Player2NPC-style persisted companion UUID mapping through `FriendMemory.companionEntityUuids`, Player2NPC-style right-handed main-arm behavior to `FriendEntity`, persisted-profile display-name fallback to `FriendEntity.getDisplayName()`, pickup-delay guarding and pickup feedback to `FriendEntity` automatic dropped-item pickup, randomized nearby safe spawn/recall placement to `CompanionLifecycle`, Player2NPC-style base movement constants to `FriendEntity` (`0.6F` step height, `0.4D` movement speed), Player2NPC-style skin-head character cards and selected-character detail-head rendering in the companion selector, selected-mainhand entity-data sync for visible client equipment, per-character skin URL entity sync, client downloaded-texture rendering through `CompanionSkinTextures`, and player-model equipment render layers for armor/arrow/spin/stinger visuals. Existing verified work includes PlayerEngine-backed `CLEAR_LIQUID` with `/staywithme clearliquid`/`clearwater`/`clearlava`, local/LLM planner routing, a limited reachable-liquid Forge fallback, automatic pre-dig liquid clearing in resource exploration and expedition passage digging through `PlayerEngineBlockSafetyRunner`, PlayerEngine `GetToYTask` before local `DESCEND_TO_LAYER` stair digging, remote/stalled return `GetToBlockTask` attempts before construction-route fallback, `COLLECT_FUEL` charcoal fallback workflow, `GET_OUT_OF_WATER` dry-stand fallback, `ESCAPE_LAVA` lava-safe-ground fallback, carried-armor `EQUIP_ARMOR` fallback, local hostile/projectile retreat fallback, carried-block projectile cover fallback, and fallback-helper splits via `LocalArmorEquipFallback`, `LocalHazardEscapeFallback`, `LocalBlockSafetyFallback`, `LocalThreatSafetyFallback`, `LocalInventoryFallback`, `LocalItemMatcher`, `PlayerEngineResumeValidator`, `PlayerEngineMovementRunner`, `PlayerEngineArmorEquipRunner`, `PlayerEngineHazardEscapeRunner`, `PlayerEnginePlaceBlockRunner`, `PlayerEngineBlockSafetyRunner`, `PlayerEngineThreatSafetyRunner`, `PlayerEngineHostileAttackRunner`, `PlayerEngineCollectionRunner`, `PlayerEngineSmeltingRunner`, `PlayerEngineRoutineRunner`, `PlayerEngineItemTransferRunner`, `PlayerEngineRemoteTravelRunner`, `PlayerEngineYLevelRunner`, `PlayerEngineExploreRunner`, `PlayerEngineFuelRunner`, `PlayerEngineConstructionMaterialRestockRunner`, `ConstructionTravelResult`, `PlayerEngineAcquisitionRequests`, `PlayerEngineAcquisitionRunner`, `PlayerEngineCountedTaskRunner`, `PlayerEngineStartOnlyTaskRunner`, `PlayerEngineConfirmedTaskRunner`, `PlayerEngineFallbackTaskRunner`, `PlayerEngineStatusText`, and `PlayerEngineTaskState` with centralized PlayerEngine start/announcement handling; `PlayerEngineHostileAttackRunner` now owns both one-shot hostile attack startup and continuous protect startup, and `LocalInventoryFallback` owns basic carried material/tool/smelt-output counters, best carried-food selection, construction floor-repair material filtering, supply-furnace block selection, free-slot counting, generic supply-container item count/move/overflow policy helpers, supply-stock status formatting, and expedition pickaxe usability/durability checks. Existing PlayerEngine-backed entries include `GET_ITEM`, `PICKUP_DROPPED_ITEM`, `COLLECT_BUILDING_MATERIALS`, construction-route material restock, `GIVE_ITEM`/`DEPOSIT_INVENTORY`, `SMELT_ITEM`, `EXPLORE`, combat/safety tasks, sleep, food/meat/fuel collection, fish/farm/equip, execution-mode fields in `WorldSnapshot`, `/staywithme capabilities`, catalogue diagnostics, generic-get mining fallback, inventory precheck, and local planner quantity parsing.
