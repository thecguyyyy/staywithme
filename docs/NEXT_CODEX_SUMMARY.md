...
- Latest focused pass: Player2NPC-style character skins are now synced through `FriendEntity` entity data and rendered on the client through `CompanionSkinTextures`, with default-skin fallback when a selected PlayerEngine character has no valid skin URL. `FriendRenderer` now uses `PlayerModel` plus armor, arrow, spin-attack, and stinger layers so equipped armor and skin outer layers render closer to Player2NPC's `RenderAutomaton`.
...
- `FriendEntity` syncs each persisted companion profile's skin URL through vanilla entity data, and `FriendRenderer` resolves that URL through `CompanionSkinTextures` so selected PlayerEngine characters render with their Player2NPC-style downloaded skins on clients. `FriendRenderer` now uses a player model plus armor/arrow/spin/stinger layers, preserving the existing head, elytra, and held-item layers from the humanoid renderer.
- `FriendEntity` and `FriendAutomatoneBridge` expose and synchronize the offhand equipment slot instead of clearing it, matching Player2NPC's player-like mainhand/offhand/armor equipment mapping more closely.
