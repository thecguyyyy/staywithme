...
- `CompanionLifecycle` mirrors Player2NPC's thin `CompanionManager` lifecycle on Forge: login ensures an owned companion exists near the player when `companion.autoSummonCompanion=true`, and logout dismisses only the auto-created session companion when `companion.dismissAutoSummonedCompanionsOnLogout=true`. Manual `/staywithme spawn` companions stay persistent unless explicitly dismissed with `/staywithme dismiss` or `/staywithme despawn`.
...
- Player2NPC-style client companion selection was added:
  ...
  - `CompanionCharacterActionPacket` sends summon/despawn requests to the server without serializing PlayerEngine classes across the common packet boundary
  ...
  - summon replaces the current owned companion so an already-initialized `PlayerEngineController` cannot keep running under the previous character identity
