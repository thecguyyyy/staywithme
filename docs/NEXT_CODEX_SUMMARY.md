...
- Latest focused pass: Player2NPC-style character skins are now synced through `FriendEntity` entity data and rendered on the client through `CompanionSkinTextures`, with default-skin fallback when a selected PlayerEngine character has no valid skin URL.
...
- `FriendEntity` syncs each persisted companion profile's skin URL through vanilla entity data, and `FriendRenderer` resolves that URL through `CompanionSkinTextures` so selected PlayerEngine characters render with their Player2NPC-style downloaded skins on clients.
